...
public class BaseTestScript extends BaseController{

	 final private Logger log = Logger.getLogger(BaseTestScript.class.getName());
	...
	public void beforeTest() throws Exception //it will make extent report ready
	{
		try {
			setDatasetNum();//set to 1
			...
			reports.addSystemInfo("Host Name","Fold").addSystemInfo("Environment","Test5").addSystemInfo("Tester","Ashlesha");
			reports.loadConfig(new File(System.getProperty("user.dir")+"//src//main//java//configuration//extent-config.xml"));
	...
	{
		return config.readData(jsonfileName, testMethod.getName());
